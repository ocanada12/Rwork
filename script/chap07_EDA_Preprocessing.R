...
# $ gender  : int  1 1 1 2 1 1 2 1 1 1 ... 범주형 (명목)
# $ job     : int  1 2 2 NA 3 2 1 2 1 2 ... 범주형 (명목)
# $ age(비율): int  26 54 41 45 62 57 36 NA 56 37 ... 연속형 (비율)
# $ position: int  2 5 4 4 5 NA 3 3 5 3 ... 범주
# $ price(비율)   : num  5.1 4.2 4.7 3.5 5 5.4 4.1 675 4.4 4.9 ... 연속
# $ survey(등간)  : int  1 2 4 2 1 2 4 4 3 3 ... 범주
...
for(i in 1:nrow(dataset)){ # index : 300 반복
 if(is.na(dataset$price[i])){ # NA
   if(dataset$type[i] == 1){ # 우수
     price4[i] = avg * 1.5
   }else if(dataset$type[i] == 2){ # 보통
     price4[i] = avg
   }else{ # 저조
     price4[i] = avg * 0.5
   }
...
# 이상치 발견
boxplot(price)$stats # 정상적인값의 범위를 보여줌 핵심
...
# 1) 데이터 가독성
table(dataset2$resident) # table 빈도수 보기
# 1   2   3   4   5 
# 131  54  30  17  44 
