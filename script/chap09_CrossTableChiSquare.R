...
#   -------------|-----------|-----------|-----------|
#   고졸 |   40 |        49 : 관측치 |        89 | 
#   |     0.544 |     0.363 : 기대비율(기대율) -> 카이스퀘어식에의해 계산된것(가설검증에 사용) |           | 
#   |     0.449 |     0.551 : 행에대한 비율 |     0.396 | 
#   |     0.444 |     0.363 : 열에대한 비율 |           | 
#   |     0.178 |     0.218 : 전체테이블에 대한 비율  |           | 
...
# (1) 적합성 검정 예
#-----------------------------------------------
# 귀무가설 : 기대치와 관찰치는 차이가 없다. => 도박사의 주사위는 게임에 적합하다.
# 대립가설 : 기대치와 관찰치는 차이가 있다. => 도박사의 주사위는 게임에 적합하지 않다.
#-----------------------------------------------
# 가설 설정 방법
...
# X-squared = 14.2, df = 5, p-value = 0.01439 < 0.05 미만 유의한 수준
# 게임에 적합하지 않은 주사위다.
...
# 0.0003999 <= 유의수준 -> 귀무가설 기각 -> 선호도간의 차이가 있다.
# x-squared >9.488 -> 귀무가설 기각
...
chisq.test(x,y)
# p-value = 6.6 *e^-9 < 0.05
# 꽃의 종류별로 꽃받팀의 길이에 차이가 있다고 할수있다.
